...
namespace TK_Timestamp_Human\Common\Utilities;
...
if ( ! class_exists( Timing::class ) ) {
	...
	class Timing {
		 ...
		 * @link https://www.php.net/manual/en/timezones.php Allowed time zones.
		 *
		 * @param int    $utc_timestamp An integer timestamp in UTC time zone.
		 * @param string $format        A valid PHP date() format.
		 * @param string $time_zone     A PHP named time zone, else WP's is used, else hard-coded fallback is used.
		 *
		 * @return DateTimeImmutable|string|bool
		 */
		public function get_datetime_from_utc_timestamp( int $utc_timestamp, string $format = '', string $time_zone = '' ) {
			if (
				empty( $utc_timestamp )
			...
			$result = $utc_datetime->setTimezone( new DateTimeZone( $time_zone ) );
